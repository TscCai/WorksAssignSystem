    ...
    public class ExportUtil {
        ...
        public void ExportExcel(string filename, DateTime startDate)
        {
            ...
            foreach (Employee emp in Db.Employees) {
                ...
                // 取出该人员的所有工作内容
                List<Vw_AllScore> personal_points = Db.GetWorkScoreById(emp.Id);

                ISheet personal_sheet = CreatePersonalSheet(emp.Name);

                while (datePointer.Month == currentMonth) {
                    // search if this day have work
                    List<Vw_AllScore> item = personal_points.Where(s => s.WorkDate == datePointer).ToList();
        ...
        double CalcPoints(List<Vw_AllScore> workOfOneDay, ISheet personalSheet)
        {
        ...
        void FillPersonalSheet(ISheet sheet, Vw_AllScore scoreTable)
        {
            FillPersonalSheet(sheet, scoreTable.WorkDate, scoreTable.WorkContent, scoreTable.WorkType,
                scoreTable.TypeWgt, scoreTable.Role, scoreTable.RoleWgt);
        }
